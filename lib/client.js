...
// classic <script>, so `import` is a SyntaxError). The family's utility launcher
// is therefore embedded below rather than imported — it is generated at build
// time from the dsh-mini-utility-dock package by `npm run launcher:sync`, so edit
// that package, never this block (`npm run launcher:check` fails on drift). The
// fragment registers the family's single launcher icon and the menu it opens, and
// contributes this plugin's row to that menu, so ballast works standalone: no
  ...
  factory: (require) => {
    ...
    function ensureStyles() {
      ...
      styleEl.textContent =
        ...
        // The Session-row hover section is one line of the row's own card, so
        // it reuses the card's type scale instead of the panel's box chrome.
        '.dshbl-hover{display:flex;align-items:baseline;gap:8px;font-size:11.5px;color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums}' +
        '.dshbl-hover-fact{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}' +
        '.dshbl-hover-open{flex:none;margin-left:auto;padding:2px 8px;border:0;border-radius:7px;background:transparent;color:var(--dsw-alias-brand-primary);font:inherit;font-size:11.5px;cursor:pointer;white-space:nowrap}' +
        '.dshbl-hover-open:hover{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary)}'
    ...
    // ---- session focus ---------------------------------------------------
    // A Session-row hover seat cannot reach the panel's own selectSession
    // closure, so the request travels through module state: the seat records
    // the session and opens the panel, and the panel's next read consumes the
    // record and targets that session. Only the id crosses, and nothing here
    // touches the network, so a hover on its own is never a host request.
    const focusStore = { sessionId: null, pending: false, revision: 0, listeners: new Set() }
    const requestSessionFocus = (sessionId) => {
      if (typeof sessionId !== 'string' || sessionId === '') return
      focusStore.sessionId = sessionId
      focusStore.pending = true
    ...
    const useFocusRevision = () => {
      ...
      return focusStore.revision
    }
    // Consumed by the read that a request triggered, at its head, so the session
    // list and the measurement that follows it stay one intent.
    const consumeFocus = () => {
      if (!focusStore.pending) return null
      focusStore.pending = false
      return focusStore.sessionId
    }

    // ---- session facts ---------------------------------------------------
    // Per-session facts the plugin has already been told by the host, keyed by
    // the sessionId the host used. Every field is written from a payload the
    // user's own read returned — never measured or estimated here — so the
    // hover seat below can answer from memory instead of paying for a fresh
    // `measure` on every row the pointer crosses. A session that has only been
    // listed carries its title and log length and no token figure at all; that
    // absence is kept as absence rather than defaulted to 0.
    const sessionFacts = new Map()
    const factsStore = { revision: 0, listeners: new Set() }
    const readSessionFacts = (sessionId) => sessionFacts.get(sessionId) || null

    const rememberSessionFacts = (sessionId, patch) => {
      if (typeof sessionId !== 'string' || sessionId === '') return
      const before = sessionFacts.get(sessionId)
      const next = { ...(before || {}) }
      const put = (key, value) => {
        if (value !== undefined && value !== null) next[key] = value
      }
      put('title', typeof patch.title === 'string' && patch.title !== '' ? patch.title : undefined)
      put('titleSource', typeof patch.titleSource === 'string' ? patch.titleSource : undefined)
      put('eventCount', finite(patch.eventCount))
      put('totalTokens', finite(patch.totalTokens))
      put('surfaceTokens', finite(patch.surfaceTokens))
      put('nodeCount', finite(patch.nodeCount))
      // Only a finite token figure makes a heaviest entry: a row the host left
      // unpriced is not the heaviest *priced* entry and must not be cached as a
      // number the cache would then hand back as one.
      const heaviest = patch.heaviest && typeof patch.heaviest === 'object' ? patch.heaviest : null
      if (heaviest && finite(heaviest.tokens) !== undefined) {
        put('heaviest', {
          seq: finite(heaviest.seq),
          type: typeof heaviest.type === 'string' ? heaviest.type : null,
          tokens: finite(heaviest.tokens)
        })
      }
      if (JSON.stringify(next) === JSON.stringify(before)) return
      sessionFacts.set(sessionId, next)
      factsStore.revision += 1
      factsStore.listeners.forEach((listener) => listener())
    }

    const useFactsRevision = () => {
      const [, setTick] = React.useState(0)
      React.useEffect(() => {
        const listener = () => setTick((value) => value + 1)
        factsStore.listeners.add(listener)
        return () => factsStore.listeners.delete(listener)
      }, [])
      return factsStore.revision
    }
    ...
    const ZH = {
      ...
      'hover.surface': 'surface {tokens}', 'hover.heaviest': '最重 {label}{tokens}',
      'hover.events': '日志 {count} 条', 'hover.open': '查看占用',
    ...
    const EN = {
      ...
      'hover.surface': 'surface {tokens}', 'hover.heaviest': 'heaviest {label}{tokens}',
      'hover.events': 'log {count} events', 'hover.open': 'Show breakdown',
    ...
    /** A finite number, or undefined: the only shape a host figure may have. */
    function finite(value) {
      return typeof value === 'number' && Number.isFinite(value) ? value : undefined
    }

    /**
     * One-line summary of the facts already cached for a session. Returns ''
     * when the cache holds no measured token figure, because the seat then
     * offers only its action: a dash or a 0 would read as a measurement this
     * plugin never took. The log length is a real host figure, so it is the
     * last resort rather than a placeholder.
     */
    function factSummary(fact) {
      if (!fact || typeof fact !== 'object') return ''
      const parts = []
      if (finite(fact.surfaceTokens) !== undefined) {
        parts.push(tr('hover.surface', { tokens: fmt(fact.surfaceTokens) }))
      }
      const heaviest = fact.heaviest
      if (heaviest && finite(heaviest.tokens) !== undefined) {
        const seq = finite(heaviest.seq)
        parts.push(tr('hover.heaviest', {
          label: seq === undefined ? '' : `#${seq} `, tokens: fmt(heaviest.tokens)
        }))
      }
      if (parts.length === 0 && finite(fact.eventCount) !== undefined) {
        parts.push(tr('hover.events', { count: fmt(fact.eventCount) }))
      }
      return parts.join(' · ')
    }
...
// </dsh-utility-launcher>
    ...
    function BallastSurface() {
      ...
      const loadMeasure = React.useCallback(async (sessionId) => {
        ...
        // Remember what the host just answered for this session so the hover
        // seat can repeat it without asking again. Every value is copied from
        // the payload; nothing is derived or estimated at this point.
        rememberSessionFacts(body.sessionId || sessionId, {
          title: body.title,
          titleSource: body.titleSource,
          eventCount: body.measurement && body.measurement.eventCount,
          totalTokens: body.measurement && body.measurement.totalTokens,
          surfaceTokens: body.measurement && body.measurement.surfaceTokens,
          nodeCount: body.measurement && body.measurement.nodeCount,
          heaviest: body.measurement && body.measurement.rows ? body.measurement.rows[0] : null
        })
      ...
      const loadTop = React.useCallback(async () => {
        ...
        // The host-wide scan already measured every live session, so its
        // per-session entries are facts this plugin now holds. Caching them
        // here is what lets a hover row answer without a scan of its own.
        for (const entry of body.sessions || []) {
          rememberSessionFacts(entry.sessionId, {
            title: entry.title,
            titleSource: entry.titleSource,
            surfaceTokens: entry.surfaceTokens,
            nodeCount: entry.nodeCount,
            heaviest: entry.rows ? entry.rows[0] : null
          })
        }
      ...
      const refresh = React.useCallback(async () => {
        ...
        // A hover action's request is another selection intent. It is taken
        // here, at the head of the read it triggered, so the session list and
        // the measurement that follows it stay one generation and one target,
        // and so opening from a hover costs exactly the one read the open
        // would have cost anyway.
        const requested = consumeFocus()
        ...
        // The list is already a host answer: its title and log length are facts
        // worth keeping even though it carries no token figure.
        for (const session of sessions) {
          rememberSessionFacts(session.sessionId, {
            title: session.title, titleSource: session.titleSource, eventCount: session.eventCount
          })
        }
        ...
        // A requested session outranks the previous selection: it is the newest
        // intent, and the read it caused must not land on another session.
        const keep = sessions.some((x) => x.sessionId === requested) ? requested
          : sessions.some((x) => x.sessionId === current.selected) ? current.selected
            : (sessions[0] || {}).sessionId || null
        const wantTop = !requested && current.view === 'host'
        setState((s) => ({
          ...s,
          ...
          sessions,
          selected: keep,
          // A hover action opens the per-entry view; the host-wide scan is
          // opt-in and is never what a row's action asked for.
          view: requested ? 'session' : s.view,
      ...
      // A hover row can ask for a session while the panel is already open. The
      // first open is covered by the effect above, whose read consumes the
      // request at its head, so this one only ever sees a request that arrived
      // with the panel already up.
      React.useEffect(() => {
        if (!open || !focusStore.pending) return
        void refresh()
      }, [open, focusRevision])
    ...
    // ---- session row hover -----------------------------------------------
    // The Session row's hover-card section. It is mounted only while a card is
    // open, so everything it shows must already be in memory: a hover can never
    // start a host read, which is why this renders what an earlier panel read
    // cached and nothing else. Without a cached fact it still offers the panel
    // entry point — that is the discovery win, and it costs nothing until the
    // click.
    function BallastSessionHover(props) {
      useLocaleRevision()
      useFactsRevision()
      const sessionId = props && typeof props.sessionId === 'string' ? props.sessionId : ''
      if (sessionId === '') return null
      const summary = factSummary(readSessionFacts(sessionId))
      return h('div', { className: 'dshbl-hover' },
        summary !== '' && h('span', { className: 'dshbl-hover-fact', title: summary }, summary),
        h('button', {
          type: 'button',
          className: 'dshbl-hover-open',
          title: tr('host.open'),
          onClick: () => requestSessionFocus(sessionId)
        }, tr('hover.open'))
      )
    }
        ...
        ctx.inject(['slots'], (scope) => {
          ...
          scope.slots.inject('shell.overlay', () => scope.slots.register(
            { name: 'shell.overlay', id: 'ballast-panel', order: 80,
              label: () => tr('dock.label'), locale: LOCALE_NS },
            () => h(BallastSurface)))
          // The Sidebar's Session rows now carry this plugin's entry point: a
          // one-line section on the row's hover card. It only ever renders on a
          // hover and only from the fact cache, so it adds no host read of its
          // own. `order` 30 keeps it clear of the shipped schedule section (10).
          scope.slots.inject('sidebar.session.row.hover', () => scope.slots.register(
            { name: 'sidebar.session.row.hover', id: 'ballast', order: 30,
              label: () => tr('dock.label'), locale: LOCALE_NS },
            BallastSessionHover))
        })
      }
