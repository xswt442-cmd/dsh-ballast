    ...
    function BallastSurface() {
      ...
      const loadMeasure = React.useCallback(async (sessionId) => {
        ...
        try {
          ...
          setState((s) => ({ ...s, measure: null, receivedAt: null, error: `measure 请求失败：${String((e && e.message) || e)}` }))
          return
        }
        if (gen !== generation.current) return
        if (!body.ok) {
      ...
      const loadTop = React.useCallback(async () => {
        ...
        try {
          ...
          if (gen !== generation.current) return
          setState((s) => ({ ...s, top: null, error: `top 请求失败：${String((e && e.message) || e)}` }))
          return
        }
        if (gen !== generation.current) return
        if (!body.ok) {
      ...
      const refresh = React.useCallback(async () => {
        ...
        try {
          ...
          // `loading` describes this read, not the panel's target, so it is
          // cleared even once superseded — otherwise the refresh button stays
          // disabled for good. The message is not: it would report a request
          // the user has already moved on from.
          setState((s) => ({ ...s, loading: false, ...(gen === generation.current ? { error: message } : {}) }))
          return
        }
