...
import { VERSION, sendJson, ballastGuard, requireGet, optionalSessionId } from './shared.js'
...
export function apply(ctx) {
  ...
  // plugin's strict loopback guard; a rejection from Connection is final.
  ...
  const apiRoute = {
    ...
    handler: async (req, res) => {
      try {
        if (connection) {
          let rejection
          // A Connection that cannot answer is not a Connection that permits:
          // a throw here used to fall through to the route handler below, which
          // then answered the very request no guard had cleared.
          ...
          // Once an RC1 Connection has existed, an unload/reload gap must not
          // reopen the route through the weaker legacy compatibility fence.
          sendJson(res, 503, { ok: false, code: 'connection_unavailable', error: 'browser authentication unavailable' })
          return
        } else if (!guard(req, res)) return
