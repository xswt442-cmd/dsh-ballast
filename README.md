...
- 在 DSH 0.1.5-rc.3 及更高版本中，API 复用 Connection 的 Host/Origin 校验和浏览器签名 cookie；缺少或错误的浏览器认证返回 `401/403`。只有未挂载 Connection 的宿主才回退到按 TCP 对端、Fetch Metadata、`Origin` 和 loopback `Host` 判定的本地守卫。写方法统一返回 `405`。
